...
const eventsStats = objectEventsStats.eventos;

// obtener array con todos los porcentajes de asistencia reales
const allPercentagesAssistence = [];

const getHighestPercentagesOfAttendance = () => {
    
    for (let i=0; i < eventsStats.length; i++) {
        if (eventsStats[i].hasOWnProperty('assistance')) {
            const eventPercentageOfAttendance = Math.round((eventsStats[i].assistance * 100) / eventsStats[i].capacity);
            console.log(eventPercentageOfAttendance)
            allPercentagesAssistence.push(eventPercentageOfAttendance);
            console.log(allPercentagesAssistence)
        }
    }
}

// renderizar en tabla porcentajes
getHighestPercentagesOfAttendance();
    const highestPercentageAttendanceInHTML = document.getElementById('highest-attendance');

const renderHighestPercentagesOfAttendance = () => {
