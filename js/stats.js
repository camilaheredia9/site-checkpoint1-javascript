...
const getDataEventsStats = async () => {
  try {
    const promise = await fetch("../js/data.json");
    const response = await promise.json();
    functionStats(response.events);
  } catch (error) {
...
function functionStats(events) {
  ...
  const getPercentagesOfAttendanceAndCapacities = (requiredData) => {
    ...
    events.forEach((event) => {
      ...
      const eventPercentageOfAssistance = Math.round(
        (assistance * 100) / event.capacity
      );
  ...
  const renderDataInFirstTable = () => {
    const eventsWithHighestAssistances = orderValues(
      getPercentagesOfAttendanceAndCapacities("assistance"),
      "percAssistance",
      "desc"
    );

    const eventsWithLowestAssistances = orderValues(
      getPercentagesOfAttendanceAndCapacities("assistance"),
      "percAssistance",
      "asc"
    ).reverse();

    const eventsWithLargerapacities = orderValues(
      getPercentagesOfAttendanceAndCapacities("capacity"),
      "capacity",
      "desc"
    );
  ...
  const getPercAssistanceByCategory = (events, category) => {
    ...
    const percAssistanceByCategory = Math.round(
      (totalAssistance * 100) / totalCapacity
    );
